  ...
  bot.addCommand("!monstercat", "Toggle Monstercat mode", function(msg, args, channel, user){
    if(admins.indexOf(user.name) !== -1){
      if(config.twitch.enable){
        ...
        user.sendMessage("Please enable Monstercat mode before using this command. Please check <a href='https://horner.tj/p/mumble-musicbot'>the GitHub page</a> for more details.");
      }
    }
  });
}
